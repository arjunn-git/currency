...
export default function CurrencyPicker({
  ...
  const modalContent = isOpen ? (
    ...
    >
      ...
      >
        {/* Modal Header */}
        <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-blue-50/70 to-indigo-50/70">
          <div>
            <h3 className="text-base sm:text-lg font-bold text-gray-900">
              Select Currency
            </h3>
            <p className="text-xs text-gray-500">
              Choose from 160+ world currencies
            </p>
          </div>
          <button
            type="button"
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <FlagIcon currencyCode={upper} className="w-7 h-5 shadow-xs" />
                    <div className="flex flex-col min-w-0">
                      <span className="text-sm font-bold flex items-center gap-1.5 text-gray-900">
                        {upper}
                        {meta.symbol && (
  ...
  return (
    <>
      ...
      <button
        ...
        className="flex items-center justify-between gap-2 px-3 py-2 bg-white/95 hover:bg-white text-gray-800 rounded-xl border border-gray-200/90 shadow-sm hover:shadow transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed group w-full text-left"
      >
        <div className="flex items-center gap-2.5 min-w-0">
          <FlagIcon currencyCode={selectedCurrency} className="w-6 h-4" />
          <div className="flex flex-col min-w-0">
            <span className="font-bold text-sm sm:text-base text-gray-900 tracking-wide leading-tight">
              {selectedCurrency.toUpperCase()}
            </span>
            <span className="text-[11px] text-gray-500 truncate max-w-[95px] sm:max-w-[120px]">
              {selectedMeta.name}
            </span>
          </div>
        </div>
      ...
      {/* Render modal directly into document.body to prevent stacking context clipping */}
